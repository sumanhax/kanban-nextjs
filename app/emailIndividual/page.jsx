...
// Simple SVG icon for accordion chevron
...
export default function EmailIndividual({ thread = [], onBack }) {
  // Defensive: Ensure thread is always an array
  const chronologicalThread = useMemo(() => Array.isArray(thread) ? [...thread].reverse() : [], [thread]);
  ...
  const handleSendReply = () => {
    ...
    alert(`Reply Sent:\n\n${replyText}`);
    setReplyText(''); // Clear textarea after sending
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <h2 className="text-xl font-bold text-gray-800">{chronologicalThread[0]?.subject || 'No Subject'}</h2>
      </div>
